...
def existing_languages():
    ...
    # keep only the '<Name>'
    return [
        lang.title().split(".")[0] for lang in listdir("langs") if lang != "__pycache__"
    ]
...
class ConfigurationPage(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        # set up frame title
        tk.Label(self, text=self.module.TITLE_INITIAL_CONFIG, font=("Ariel", 24)).pack(
            side="top", pady=10
        )

        # ask for directory and show selected path
        tk.Label(self, text=f"{self.module.TITLE_GIVE_INBOX}:").pack(side="top", pady=5)
        ...
        ttk.Button(
            self,
            text=f"{self.module.TITLE_OPEN_FE}...",
            padding=5,
        ...
        self.to_date_entry = DateEntry(
            self, date_pattern="yyyy-mm-dd", width=12, allow_none=True
        )
        ...
        tk.Label(
            self,
            text=f"{self.module.TITLE_GIVE_USERNAME}:",
    ...
    def open_file_explorer(self):
        ...
        self.directory_label.config(
            text=(
                self.module.TITLE_NO_SELECTION
                if path == "" or path.isspace() or path == "/"
                else path
            )
        )
...
class MainPage(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        self.style.configure(
            "Custom.Treeview", background="#232323", foreground="#ffffff"
        )
        ...
        self.treeview = ttk.Treeview(
            self.main, height=20, yscrollcommand=scrollbar.set, style="Custom.Treeview"
        )
        columns = {
        ...
        ttk.Label(
            self.main,
            text=f"{self.module.TITLE_NUMBER_OF_MSGS}: ",
            foreground="#ffffff",
            background="#232323",
            font=("Arial", 15),
        ).pack(side="top", pady=10)
        ...
        ttk.Button(
            self.nav,
            image=self.controller.ICON_HOME,
            text=self.module.TITLE_HOME,
            compound="left",
            padding=5,
        ).pack(side="top", pady=10)
        ...
        ttk.Button(
            self.nav,
            image=self.controller.ICON_STATUS_VISIBLE,
            text=self.module.TITLE_UPLOAD_MESSAGES,
            compound="left",
            padding=5,
        ...
        ttk.Button(
            self.nav,
            image=self.controller.ICON_SEARCH,
            text=self.module.TITLE_SEARCH,
            compound="left",
            command=self.search,
        ).pack(side="top", pady=10)
        ...
        ttk.Button(
            self.nav,
            image=self.controller.ICON_SETTINGS,
            text=self.module.TITLE_SETTINGS,
            compound="left",
            padding=5,
            command=lambda: SettingsPopup(self.controller),
        ).pack(side="bottom", pady=15)

        # show profile button
        ttk.Button(
            self.nav,
            image=self.controller.ICON_PROFILE,
            text=self.module.TITLE_PROFILE,
            compound="left",
            padding=5,
            command=lambda: ProfilePopup(self.controller),
        ).pack(side="bottom")
    ...
    def upload_data(self):
        ...
        try:
            ...
            self.treeview.heading(
                "msg",
                command=lambda col="msg": self.sort_treeview(col, False, "numberwise"),
            )
            self.treeview.heading(
                "name",
                command=lambda col="name": self.sort_treeview(col, False, "stringwise"),
            )
            self.treeview.heading(
                "type",
                command=lambda col="type": self.sort_treeview(col, False, "stringwise"),
            )
            self.treeview.heading(
                "call",
                command=lambda col="call": self.sort_treeview(col, False, "numberwise"),
            )
            self.treeview.heading(
                "photos",
                command=lambda col="photos": self.sort_treeview(
                    col, False, "numberwise"
                ),
            )
        except FileNotFoundError:
            print(
                ">MainPage/upload_data THROWS FileNotFoundError, NOTIFY OP IF UNEXPECTED"
            )
    ...
    def sort_treeview(self, column, order, bias):
        ...
        self.treeview.heading(
            column, command=lambda: self.sort_treeview(column, not order, bias)
        )

    # invoked on double left click on any treeview listing
    def show_statistics(self):
        try:
            selection = self.treeview.item(self.treeview.selection()[0]).get(
                "values", []
            )
            if len(selection) == 0:
                return
...
class MasterWindow(tk.Tk):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.show_frame(
            MainPage.__name__ if exists("config.txt") else ConfigurationPage.__name__
        )
    ...
    def get_username(self):
        ...
        return (
            self.lang_mdl.TITLE_NOT_APPLICABLE
    ...
    def get_directory(self):
        # noinspection PyUnresolvedReferences
        return (
            self.lang_mdl.TITLE_NO_SELECTION
    ...
    def update_data(
        self, username, directory, language, from_date_entry, to_date_entry
    ):
        temp = self.language
    ...
    def extract_data(self, conversation):
        ...
        if not isinstance(self.from_date_entry, date) and not isinstance(
            self.to_date_entry, date
        ):
            self.from_date_entry = (
                datetime.strptime(self.from_date_entry, "%Y-%m-%d").date()
                if not isinstance(self.from_date_entry, date)
                else self.from_date_entry
            )
            self.to_date_entry = (
                datetime.strptime(self.to_date_entry, "%Y-%m-%d").date()
                ...
                for message in data.get("messages", []):
                    if (
                        ...
                        try:
                            ...
                            pass
                        sender = (
                            message["sender_name"].encode("iso-8859-1").decode("utf-8")
                        )
                        ...
                        start_date = message[
                            "timestamp_ms"
                        ]  # BUG: doesn't work properly if there are 10 or more JSONs
...
class ProfilePopup(tk.Toplevel):
    def __init__(self, controller):
        ...
        # show 'My data' header
        ttk.Label(self, text=f"{self.module.TITLE_MY_DATA}:", font=("Ariel", 24)).pack(
            side="top", pady=20
        )
        ...
        try:
            ...
            conversations = 0
            print(
                ">ProfilePage/#CONVERSATIONS CALCULATION THROWS FileNotFoundError, NOTIFY OP IF UNEXPECTED"
            )
        ttk.Label(
        ...
        ttk.Label(
            self,
            text=f"{self.module.TITLE_SENT_MESSAGES}: {self.controller.sent_messages}",
        ).pack(side="top", pady=10)
...
class SettingsPopup(tk.Toplevel):
    def __init__(self, controller):
        ...
        # ask for directory and show selected path
        tk.Label(self, text=f"{self.module.TITLE_GIVE_INBOX}:").pack(
            side="top", pady=16
        )
        ...
        # ask for Facebook name
        tk.Label(self, text=f"{self.module.TITLE_GIVE_USERNAME}:").pack(
            side="top", pady=15
        )
        self.username_label = ttk.Entry(self, width=25)
        ...
        if not date_entry:
            ...
            isinstance(date_entry, tuple)
            and len(date_entry) == 1
            and isinstance(date_entry[0], date)
        ):
        ...
        self.from_date_entry = DateEntry(
            ...
            width=12,
            allow_none=True,
            year=load_from_date.year,
            month=load_from_date.month,
            day=load_from_date.day,
        )
        ...
        self.to_date_entry = DateEntry(
            ...
            width=12,
            allow_none=True,
            year=load_to_date.year,
            month=load_to_date.month,
            day=load_to_date.day,
        )
...
class LoadingPopup(tk.Toplevel):
    def __init__(self, controller, chat_total, treeview):
        ...
        if (
            self.directory != ""
            and not self.directory.isspace()
            and self.directory != self.module.TITLE_NO_SELECTION
        ):
            ...
            for conversation in listdir(self.directory):
                try:
                    (
                        ...
                        total_videos,
                        total_files,
                    ) = self.controller.extract_data(conversation)
                    if len(people) == 0:
                    ...
                    self.progress_label[
                        "text"
                    ] = f"{self.module.TITLE_LOADING_CHAT} {int(progress_value)}/{chat_total}"
...
class StatisticsPopup(tk.Toplevel):
    def __init__(self, controller, selection):
        ...
        (
            ...
            total_videos,
            total_files,
        ) = self.controller.extract_data(selection)
        ...
        # display popup title
        ttk.Label(self, text=f"{self.module.TITLE_MSG_STATS}:").pack(
            side="top", pady=16
        )
        # show conversation title and type
        ttk.Label(self, text=f"{self.module.TITLE_NAME}: {title}").pack(
            side="top", pady=5
        )
        ttk.Label(self, text=f"{self.module.TITLE_CONVERSATION_TYPE}: {room}").pack(
            side="top", pady=5
        )
        ...
        ttk.Label(
            self,
            text=f"{self.module.TITLE_PEOPLE}({len(people)}) {self.module.TITLE_AND_MESSAGES}: ",
        ).pack(side="top", pady=5)
        if room == self.module.TITLE_GROUP_CHAT:
        ...
        ttk.Label(self, text=f"{self.module.TITLE_NUMBER_OF_MSGS}: {all_msgs}").pack(
            side="top", pady=5
        )
        ttk.Label(self, text=f"{self.module.TITLE_TOTAL_CHARS}: {all_chars}").pack(
            side="top", pady=5
        )
        ttk.Label(
            self, text=f"{self.module.TITLE_NUMBER_OF_PHOTOS}: {total_photos}"
        ).pack(side="top", pady=5)
        ttk.Label(self, text=f"{self.module.TITLE_NUMBER_OF_GIFS}: {total_gifs}").pack(
            side="top", pady=5
        )
        ttk.Label(
            self, text=f"{self.module.TITLE_NUMBER_OF_VIDEOS}: {total_videos}"
        ).pack(side="top", pady=5)
        ttk.Label(
            self, text=f"{self.module.TITLE_NUMBER_OF_FILES}: {total_files}"
        ).pack(side="top", pady=5)
        ttk.Label(
            self,
            text=f"{self.module.TITLE_CALL_DURATION}: {timedelta(seconds=calltime)}",
        ).pack(side="top", pady=5)
        # show first message date
        ttk.Label(
            self,
            text=f"{self.module.TITLE_START_DATE}: {datetime.fromtimestamp(start_date / 1000)}",
        ).pack(side="top", pady=5)

        # show average messages per time period
        sec_since_start = int(time() - start_date / 1000)
        ttk.Label(self, text=f"{self.module.TITLE_AVERAGE_MESSAGES}: ").pack(
            side="top", pady=5
        )
        ...
        listbox.insert(
            "end",
            f"{self.module.TITLE_PER_DAY} - {all_msgs / (sec_since_start / 86400):.2f}",
        )
        listbox.insert(
            "end",
            f"{self.module.TITLE_PER_WEEK} - {all_msgs / (sec_since_start / (7 * 86400)):.2f}",
        )
        listbox.insert(
            "end",
            f"{self.module.TITLE_PER_MONTH} - {all_msgs / (sec_since_start / (30 * 86400)):.2f}",
        )
        listbox.insert(
            "end",
            f"{self.module.TITLE_PER_YEAR} - {all_msgs / (sec_since_start / (365 * 86400)):.2f}",
        )
